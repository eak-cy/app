...
ScalaMock's `mock[T]` macro cannot correctly mock a method shaped like `def m[A](...)(using Schema[A], JsonValueCodec[A]): F[A]` (a type parameter plus typeclass `using` parameters depending on it) — e.g. `AIClient.extractFromImage`/`OpenAIClient.sendMessage`. Verified against ScalaMock 7.5.5's own `MockMaker.scala` source, not a syntax mistake: the macro's generated override erases `A` to `Any`, so the `using` clause becomes `Schema[Any]`/`JsonValueCodec[Any]` in the generated class, which then fails to resolve (or resolves ambiguously against unrelated concrete givens) rather than picking up the real `Schema[ExtractCustomersFromPhotoResponse]`/`JsonValueCodec[ExtractCustomersFromPhotoResponse]` the call site actually needs. This is the same reason this codebase's one prior attempt to test `OpenAIClient.sendMessage[A]` (in `ReplyingToMessagesCronJobStreamSpec`) is left fully commented out rather than using `mock[OpenAIClient]`.

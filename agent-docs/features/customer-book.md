...
`AIClient` normalizes Tapir's general-purpose JSON Schema to OpenAI's strict subset before sending it: every object sets `additionalProperties: false`, every declared property is required, Scala `Option` values remain nullable, and unsupported root/default metadata is omitted while supported Iron constraints are preserved. `AIClientSchemaSpec` pins those rules, and the real-API golden spec has verified that OpenAI accepts the schema and can generate/decode all ten sample photos; a later repeated run also demonstrated that the external connection can still reset independently of schema validity.
...
- AI client: `clients/AIClient.scala` (strict schema normalization, per-attempt timeout, and selective retry policy; parallel to `clients/OpenAIClient.scala` — that client and its config are untouched), `config/AIClientConfig.scala`
...
- JSON: `json/json.scala` (`Schema`/`JsonValueCodec` givens for `ExtractCustomersResponse`, plus the bottom-up nested-type givens it needs)
...
- Integration: `it/AIClientSpec.scala` (mirrors `TwilioClientSpec`) against `src/test/resources/compose/wiremock.yaml` and the new `backend/wiremock/mappings/ai-client-chat-completions*.json` stubs — success decode, retryable HTTP/timeout failures with attempt counts, non-retryable request/decode failures, and retry exhaustion
- Unit: `AIClientSchemaSpec` pins the required OpenAI strict-schema normalization against the exact extraction response type and must also pin the two-field extracted-phone schema; `FileScannerSpec` remains unchanged because this endpoint adds no new size/type-check behavior.
...
**Status (photo extraction):** the pipeline, strict-schema normalization, dedicated extraction model tree, two-field extracted-phone shape, prompt rules, and schema/client/functional coverage are implemented. The real-API golden suite previously verified strict-schema acceptance; it still needs to be rerun after this model correction. The focused acceptance code compiles, but its latest local run aborted before executing tests because the shared acceptance harness supplied a null context. `AIClient` has its own `config/AIClientConfig.scala` and `ai-client` configuration, `json/json.scala` carries the bottom-up `Schema`/`JsonValueCodec` givens, `Main.scala` provides the client layers, and `POST /extract/customer-book-photo` is routed through `tapir/FileServiceEndpoints.scala`.
